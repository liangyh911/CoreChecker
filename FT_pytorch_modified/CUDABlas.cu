...
#include "cutlass/gemm/device/gemm_batched_std_abft.h"
#include "cutlass/gemm/device/gemm_std_abft.h"
...
__device__ Dst half_convert_device(Src x) {
    Dst f;
    if constexpr (std::is_same_v<Src, c10::Half>) {
        float tmp = x;
        f = cutlass::half_t(tmp);
    } else if constexpr (std::is_same_v<Src, cutlass::half_t>) {
...
__global__ void vector_to_matrix(Dtype *vector, Dtype *matrix, int64_t row, int64_t col){
  int colIdx = blockIdx.x * blockDim.x + threadIdx.x;
	if(colIdx < col){
		for(int r = 0; r < row; r++){
			matrix[colIdx * row + r] = vector[r];
		}
	}
}

template <typename Dtype>
__global__ void copy_matrix(Dtype *src, Dtype *dst, int64_t rows, int64_t cols){
...
__global__ void copy_matrix_2(SrcDtype *src, DstDtype *dst, int64_t rows, int64_t cols){
  int col = blockIdx.x * blockDim.x + threadIdx.x;
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  if (row < rows && col < cols) {
    int idx = row * cols + col;
    // int idx = col * rows + row;
    dst[idx] = half_convert_device<SrcDtype, DstDtype>(src[idx]);
  }
}

template <typename Dtype>
__global__ void copy_batched_matrix(Dtype *src, Dtype *dst, int64_t rows, int64_t cols, int64_t strideSrc, int64_t strideDst){
  int col = blockIdx.x * blockDim.x + threadIdx.x;
  int row = blockIdx.y * blockDim.y + threadIdx.y;
...
template <typename Dtype>
bool cutlass_bgemm_T_baseline(char transa, char transb, int64_t m, int64_t n, int64_t k, Dtype alpha,  
  ...
  // if(DEBUG){
  //   cudaEventCreate(&abft_prepare_start);
  //   cudaEventCreate(&abft_prepare_end);
  //   cudaEventRecord(abft_prepare_start, stream_main);
  // }
  ...
  // define CUTLASS GemmBatchedBaseline API
  ...
  // if(DEBUG){
  //   cudaEventRecord(abft_prepare_end, stream_main);
...
template <typename Dtype>
bool cutlass_bgemm_baseline(char transa, char transb, int64_t m, int64_t n, int64_t k, Dtype alpha,  
                        Dtype *a, int64_t lda, int64_t stridea,                                         
                        Dtype *b, int64_t ldb, int64_t strideb,                                           
                        Dtype beta, Dtype *c, int64_t ldc, int64_t stridec, int64_t num_batches,
                        bool DEBUG, int if_split_phase, char *job_id, int gpu_dev){
...
template <typename Dtype>
bool cutlass_bgemm_T_std_abft(char transa, char transb, int64_t m, int64_t n, int64_t k, Dtype alpha,  
                        Dtype *a, int64_t lda, int64_t stridea,                                         
                        Dtype *b, int64_t ldb, int64_t strideb,                                           
                        Dtype beta, Dtype *c, int64_t ldc, int64_t stridec, int64_t num_batches, 
  ...
  copy_batched_matrix<<<dim3((m+16-1)/16, (k+16-1)/16, num_batches), dim3(16, 16), 0, stream_main>>>(a_, A, k, m, stridea, stridea);
  copy_batched_matrix<<<dim3((n+16-1)/16, (k+16-1)/16, num_batches), dim3(16, 16), 0, stream_main>>>(b_, B, k, n, strideb, strideb_check);
  ...
  if(if_split_phase == 0){
    ...
    if constexpr (std::is_same<Dtype, float>::value) {
      ...
      // outputChk(dB_rowchk, num_batches, ldb, (k * 2), k, 2);
    }
    // batch wise
    copy_batched_matrix<<<dim3((2+16-1)/16, (k+16-1)/16, num_batches), dim3(16, 16)>>>(dB_rowchk, (B+(k*n)), k, 2, (k*2), strideb_check);
    // block wise
  ...
  using Gemm = cutlass::gemm::device::GemmBatchedStdABFT<
      Dtype, LayoutInputA,
      Dtype, LayoutInputB,
      Dtype, LayoutOutput,
      DtypeAccumulator,
  ...
  // Copy Back
  copy_batched_matrix<<<dim3((n+16-1)/16, (m+16-1)/16, num_batches), dim3(16, 16), 0, stream_main>>>(C, c, m, n, stridec_check, stridec);

  // printf("C:\n");
  // outputChk(C, num_batches, ldc, stridec_check, m, n1);
  // printf("c:\n");
...
template <typename Dtype>
bool cutlass_bgemm_std_abft(char transa, char transb, int64_t m, int64_t n, int64_t k, Dtype alpha,  
                        Dtype *a, int64_t lda, int64_t stridea,                                         
                        Dtype *b, int64_t ldb, int64_t strideb,                                           
                        Dtype beta, Dtype *c, int64_t ldc, int64_t stridec, int64_t num_batches,
                        bool DEBUG, int if_split_phase, bool adaptive_mod, int banned_smid,
  ...
  copy_batched_matrix<<<dim3((k+16-1)/16, (m+16-1)/16, num_batches), dim3(16, 16), 0, stream_main>>>(a_, A, m, k, stridea, stridea);
  copy_batched_matrix<<<dim3((n+16-1)/16, (k+16-1)/16, num_batches), dim3(16, 16), 0, stream_main>>>(b_, B, k, n, strideb, strideb_check);
...
bool cutlass_bgemm(char transa, char transb, int64_t m, int64_t n, int64_t k, Dtype alpha,  
  ...
  //   destinationFile = fs::path("./control_" + std::string(job_id) + "/" + std::to_string(gpu_dev)) / "time/preparation.txt";
  //   recordTime(destinationFile, t_cpu, DEBUG);
  //   cudaEventRecord(abft_prepare_start, stream_main);
  // }
  ...
  //   cudaEventRecord(abft_prepare_end, stream_main);
  //   cudaEventSynchronize(abft_prepare_end);
  //   cudaEventElapsedTime(&t_gpu, abft_prepare_start, abft_prepare_end);
  //   destinationFile = fs::path("./control_" + std::string(job_id) + "/" + std::to_string(gpu_dev)) / "time/preparation.txt";
  //   recordTime(destinationFile, t_gpu, DEBUG);
  // }
  ...
  cutlass::Status status = gemm_op({
    ...
    if_split_phase, adaptive_mod, banned_smid, transa, DEBUG, stream_main
  );
...
bool cutlass_bgemm_launcher(char transa, char transb, int64_t m, int64_t n, int64_t k, at::opmath_type<Dtype> alpha,  
  ...
  int banned_smid = -1;
  destinationFile = fs::path("./control_" + std::string(job_id) + "/" + std::to_string(gpu_dev)) / "banned_smid.txt";
  std::ifstream bannedSMFile(destinationFile);
  if(bannedSMFile.is_open()){
    if (bannedSMFile >> banned_smid) {
        // std::cout << "banned sm = " << banned_smid << std::endl;
    }
    else {
      banned_smid = -1;
    }
    ...
    printf("Banned SM: Cannot open file, using default setting.\n");
  }
  bannedSMFile.close();
  ...
  if constexpr (std::is_same<Dtype, at::BFloat16>::value) {
    ...
    if(transa == 't'){
      ...
      else if(core_checker_flag == 's' || core_checker_flag == 'a'){
        state = cutlass_bgemm_T_std_abft<cutlass::bfloat16_t>(transa, transb, m, n, k, alpha_,  
          a_, lda, stridea,                                         
          b_, ldb, strideb,                                           
          beta_, c_, ldc, stridec, num_batches, DEBUG, if_split_phase, adaptive_mod, banned_smid, job_id, gpu_dev);
      }
      else{
      ...
      if(core_checker_flag == 'b'){        
        ...
        state = cutlass_bgemm_std_abft<cutlass::bfloat16_t>(transa, transb, m, n, k, alpha_,  
          a_, lda, stridea,                                         
          b_, ldb, strideb,                                           
          beta_, c_, ldc, stridec, num_batches, DEBUG, if_split_phase, adaptive_mod, banned_smid, job_id, gpu_dev);
      }
...
template <typename Dtype>
bool cutlass_gemm_baseline(char transa, char transb, int64_t m, int64_t n, int64_t k, Dtype alpha,
  ...
  using Gemm = cutlass::gemm::device::GemmBaseline<Dtype,
                                         LayoutInputA,
                                         Dtype,
                                         LayoutInputB,
                                         Dtype,
  ...
  // printf("launch\n");
  status = gemm_op(DEBUG, stream_main);
  CUTLASS_CHECK(status);
...
template <typename Dtype>
bool cutlass_gemm_std_abft(char transa, char transb, int64_t m, int64_t n, int64_t k, Dtype alpha,
                  Dtype *a, int64_t lda, Dtype *b, int64_t ldb, Dtype beta,
  ...
  int const count_B = ldb * n1;
  int const count_C = ldc * n1;
  ...
  copy_matrix<<<dim3((m + 16 - 1) / 16, (k + 16 - 1) / 16), dim3(16,16), 0, stream_main>>>(a_, A, k, m);
  copy_matrix<<<dim3((n + 16 - 1) / 16, (k + 16 - 1) / 16), dim3(16,16), 0, stream_main>>>(b_, B, k, n);
  ...
  if(if_split_phase == 0){
    ...
    if constexpr (std::is_same<Dtype, float>::value) {
      ...
      float beta_bf16 = static_cast<float>(beta);
      // matrix wise
      cublasGemmEx(handle_main, CUBLAS_OP_N, CUBLAS_OP_N, k, 2, n,
                                    &alpha_bf16, reinterpret_cast<__nv_bfloat16*>(B), CUDA_R_16BF, ldb,
                                    reinterpret_cast<__nv_bfloat16*>(d_chk_vector), CUDA_R_16BF, n, &beta_bf16, 
                                    reinterpret_cast<__nv_bfloat16*>(B+(ldb*n)), CUDA_R_16BF, ldb,
                                    CUDA_R_32F,
                                    CUBLAS_GEMM_DEFAULT_TENSOR_OP);
    }
    cudaFree(d_chk_vector);
  ...
  // define CUTLASS GEMM API
  using Gemm = cutlass::gemm::device::GemmStdABFT<Dtype,
                                         LayoutInputA,
                                         Dtype,
                                         LayoutInputB,
                                         Dtype,
  ...
  // Copy results back
  // printf("copy back\n");
  copy_matrix<<<dim3((n + 16 - 1) / 16, (m + 16 - 1) / 16), dim3(16,16), 0, stream_main>>>(C, c, m, n);
  
  // result_ptr = tensor_d.device_data();
  // printf("C:\n");
  // outputChk(C, 1, ldc, m*n1, m, n1);
...
bool cutlass_gemm(char transa, char transb, int64_t m, int64_t n, int64_t k, Dtype alpha,
  ...
  Dtype *b_ = const_cast<Dtype*>(b);
  // issue loading A?
  // copy_matrix<<<dim3((m + 16 - 1) / 16, (k + 16 - 1) / 16), dim3(16,16), 0, stream_main>>>(a_, tensor_a.device_data(), k, m);
  // copy_matrix<<<dim3((n + 16 - 1) / 16, (k + 16 - 1) / 16), dim3(16,16), 0, stream_main>>>(b_, tensor_b.device_data(), k, n);
  ...
  // printf("B_after\n");
  // outputChk(tensor_b.device_data(), 1, ldb, k*n1, k, n1);
  ...
  // Launch initialized CUTLASS kernel
  // printf("launch\n");
  status = gemm_op(if_split_phase, adaptive_mod, banned_smid, partition, DEBUG, stream_main);
  CUTLASS_CHECK(status);
...
bool cutlass_gemm_launcher(char transa, char transb, int64_t m, int64_t n, int64_t k, at::opmath_type<Dtype> alpha,
  ...
  if constexpr (std::is_same<Dtype, float>::value) {
    ...
    state = cutlass_gemm<float>(transa, transb, m, n, k, alpha_,
                                a_, lda, b_, ldb, beta_,
                                c, ldc, DEBUG, if_split_phase, adaptive_mod, banned_smid, job_id, gpu_dev);
  }
  else if constexpr (std::is_same<Dtype, at::BFloat16>::value) {
    // printf("using c10::BFloat16\n");
    ...
    if(core_checker_flag == 'b'){
      ...
      state = cutlass_gemm_std_abft<cutlass::bfloat16_t>(transa, transb, m, n, k, alpha_,
                                                a_, lda, b_, ldb, beta_,
                                                c_, ldc, DEBUG, if_split_phase, adaptive_mod, banned_smid, job_id, gpu_dev);
    }
    else{
...
bool cutlass_gemm_and_bias(bool transpose_mat1,
  ...
  // printf("print bias:\n");
  // outputChk(tensor_c.device_data(), 1, result_ld, m*n1, m, n1);
...
template <typename Dtype>
bool cutlass_gemm_and_bias_launcher(bool transpose_mat1,
    bool transpose_mat2,
    int64_t m,
    int64_t n,
    int64_t k,
...
template bool cutlass_gemm_and_bias_launcher<float>(bool transpose_mat1, bool transpose_mat2, int64_t m, int64_t n, int64_t k, at::opmath_type<float> alpha_val,
  const float* mat1_ptr, int64_t mat1_ld, const float* mat2_ptr, int64_t mat2_ld, const float* bias, float* result_ptr, int64_t result_ld,
  GEMMAndBiasActivationEpilogue activation);

template bool cutlass_gemm_and_bias_launcher<double>(bool transpose_mat1, bool transpose_mat2, int64_t m, int64_t n, int64_t k, at::opmath_type<double> alpha_val,
  const double* mat1_ptr, int64_t mat1_ld, const double* mat2_ptr, int64_t mat2_ld, const double* bias, double* result_ptr, int64_t result_ld,
  GEMMAndBiasActivationEpilogue activation);

template bool cutlass_gemm_and_bias_launcher<c10::Half>(bool transpose_mat1, bool transpose_mat2, int64_t m, int64_t n, int64_t k, at::opmath_type<c10::Half> alpha_val,
  const c10::Half* mat1_ptr, int64_t mat1_ld, const c10::Half* mat2_ptr, int64_t mat2_ld, const c10::Half* bias, c10::Half* result_ptr, int64_t result_ld,
  GEMMAndBiasActivationEpilogue activation);

template bool cutlass_gemm_and_bias_launcher<c10::BFloat16>(bool transpose_mat1, bool transpose_mat2, int64_t m, int64_t n, int64_t k, at::opmath_type<c10::BFloat16> alpha_val,
  const c10::BFloat16* mat1_ptr, int64_t mat1_ld, const c10::BFloat16* mat2_ptr, int64_t mat2_ld, const c10::BFloat16* bias, c10::BFloat16* result_ptr, int64_t result_ld,
  GEMMAndBiasActivationEpilogue activation);

/*Pytorch original gemm_bias (CUBLASS)*/
...
bool gemm_and_bias(
  ...
  // Dtype *bias_ = const_cast<Dtype*>(bias);
  // Dtype *bias_matrix;
  // cudaMalloc((void**)&bias_matrix, m*n*sizeof(Dtype));
  // vector_to_matrix<<<dim3(128), dim3((n+128-1)/128)>>>(bias_, bias_matrix, m, n);
  // printf("print bias:\n");
  // outputChk(bias_matrix, 1, result_ld, m*n, m, n);
