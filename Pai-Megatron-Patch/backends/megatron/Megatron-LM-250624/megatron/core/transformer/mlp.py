...
class MLP(MegatronModule):
    ...
    def forward(self, hidden_states, per_token_scale=None):
        ...
        #     # faults_count = ((torch.abs(hidden_states) > threshold) | torch.isnan(hidden_states) | torch.isinf(hidden_states)).sum().item()
        #     n_inf = (torch.abs(hidden_states) > threshold).sum().item()
        #     nan = (torch.isnan(hidden_states)).sum().item()
        #     inf = (torch.isinf(hidden_states)).sum().item()
        ...
        if RecFaults:
            code = "UP"
            threshold = 1e3
            count = intermediate_parallel.numel()
            # faults_count = ((torch.abs(intermediate_parallel) > threshold) | torch.isnan(intermediate_parallel) | torch.isinf(intermediate_parallel)).sum().item()
            n_inf = (torch.abs(intermediate_parallel) > threshold).sum().item()
            nan = (torch.isnan(intermediate_parallel)).sum().item()
            inf = (torch.isinf(intermediate_parallel)).sum().item()
        ...
        if Val_Range:
            record_value_range(intermediate_parallel, "UP", gpu)

        # if parallel_state.get_tensor_model_parallel_rank() == 0: 
        #     print(f"hidden_states: {hidden_states.size()}, intermediate_parallel: {intermediate_parallel.size()}")
        # if parallel_state.get_tensor_model_parallel_rank() == 0:
        #     data = intermediate_parallel
        #     mean_val = torch.mean(data)
        #     max_val = torch.max(data)
        #     min_val = torch.min(data)
        #     print(f"UP: mean: {mean_val}, min: {min_val}, max: {max_val}")
        ...
        if Val_Range:
            record_value_range(output, "DO", gpu)

        # if parallel_state.get_tensor_model_parallel_rank() == 0: 
        #     print(f"intermediate_parallel: {intermediate_parallel.size()}, output: {output.size()}")
        # if parallel_state.get_tensor_model_parallel_rank() == 0:
        #     data = output
        #     mean_val = torch.mean(data)
        #     max_val = torch.max(data)
        #     min_val = torch.min(data)
        #     print(f"DO: mean: {mean_val}, min: {min_val}, max: {max_val}")
