...
class DotProductAttention(MegatronModule):
    ...
    def forward(
        ...
        if RecFaults:
            code = "QK"
            threshold = 1e3
            count = matmul_result.numel()
            # faults_count = ((torch.abs(matmul_result) > threshold) | torch.isnan(matmul_result) | torch.isinf(matmul_result)).sum().item()
            n_inf = (torch.abs(matmul_result) > threshold).sum().item()
            nan = (torch.isnan(matmul_result)).sum().item()
            inf = (torch.isinf(matmul_result)).sum().item()
        ...
        if Val_Range:
            record_value_range(matmul_result, "QK", gpu)
        # if parallel_state.get_tensor_model_parallel_rank() == 0:
        #     data = matmul_result
        #     mean_val = torch.mean(data)
        #     max_val = torch.max(data)
        #     min_val = torch.min(data)
        #     print(f"QK: mean: {mean_val}, min: {min_val}, max: {max_val}")
        # if parallel_state.get_tensor_model_parallel_rank() == 0: 
        #     print(f"query: ({query.size()}->{query.transpose(0, 1).size()}), key: ({key.size()}->{key.transpose(0, 1).transpose(1, 2).size()}), AS: {matmul_result.size()}")
        ...
        if RecFaults:
            ...
            threshold = 1e3
            count = context.numel()
            # faults_count = ((torch.abs(context) > threshold) | torch.isnan(context) | torch.isinf(context)).sum().item()
            n_inf = (torch.abs(context) > threshold).sum().item()
            nan = (torch.isnan(context)).sum().item()
            inf = (torch.isinf(context)).sum().item()
        ...
        if Val_Range:
            record_value_range(context, "AV", gpu)
        # if parallel_state.get_tensor_model_parallel_rank() == 0:
        #     data = context
        #     mean_val = torch.mean(data)
        #     max_val = torch.max(data)
        #     min_val = torch.min(data)
        #     print(f"AV: mean: {mean_val}, min: {min_val}, max: {max_val}")
        # if parallel_state.get_tensor_model_parallel_rank() == 0: 
        #     print(f"attention_probs: {attention_probs.size()}, value: ({value.size()}->{value.transpose(0, 1).size()}), context: {context.size()}")
