...
def record_value_range(input_tensor, code, gpu):
    job_id = os.getenv('SLURM_JOB_ID')
    value_range_FP = f"./control_{job_id}/{gpu}/value_range.bin"
...
class Attention(MegatronModule, ABC):
    ...
    def forward(
        ...
        *,
        ...
        cutlassFP = f"./control_{job_id}/{gpu}/cutlass.txt"
        FIFP = f"./control_{job_id}/{gpu}/FI.txt"
        component = f"./control_{job_id}/{gpu}/component.txt"
        ...
        if RecFaults:
            code = "WA"
            threshold = 1e3
            count = query.numel() + key.numel() + value.numel()
            # faults_count = (((torch.abs(query) > threshold) | torch.isnan(query) | torch.isinf(query)).sum().item()) + \
            #                 (((torch.abs(key) > threshold) | torch.isnan(key) | torch.isinf(key)).sum().item()) + \
            #                 (((torch.abs(value) > threshold) | torch.isnan(value) | torch.isinf(value)).sum().item())
            n_inf = (torch.abs(query) > threshold).sum().item() + (torch.abs(key) > threshold).sum().item() + (torch.abs(value) > threshold).sum().item()
            nan = (torch.isnan(query)).sum().item() + torch.isnan(key).sum().item() + torch.isnan(value).sum().item()
            inf = (torch.isinf(query)).sum().item() + torch.isinf(key).sum().item() + torch.isinf(value).sum().item()

            with open(CurStepFP, 'r') as file:
                current_step = int(file.readline())
            with open(FaultsPrecentageFP, 'ab') as f:
                f.write(struct.pack('<i', current_step))
                f.write(code.encode('utf-8'))
                f.write(struct.pack('<i', count))
                # f.write(struct.pack('<i', faults_count))
                f.write(struct.pack('<i', n_inf))
                f.write(struct.pack('<i', nan))
                f.write(struct.pack('<i', inf))
        
        if Val_Range:
            record_value_range(query, "WQ", gpu)
            record_value_range(key, "WK", gpu)
            record_value_range(value, "WV", gpu)
        # ===================================================
        ...
        if RecFaults:
            ...
            threshold = 1e3
            count = output.numel()
            # faults_count = ((torch.abs(output) > threshold) | torch.isnan(output) | torch.isinf(output)).sum().item()
            n_inf = (torch.abs(output) > threshold).sum().item()
        ...
        #     print(f"WO: mean: {mean_val}, min: {min_val}, max: {max_val}")

        # if get_tensor_model_parallel_rank() == 0: 
        #     print(f"core_attn_out: {core_attn_out.size()}, output: {output.size()}")
...
class SelfAttention(Attention):
    ...
    def get_query_key_value_tensors(self, hidden_states, key_value_states=None):
        ...
        #     max_val = torch.max(data)
        #     min_val = torch.min(data)
        #     print(f"QKV: mean: {mean_val}, min: {min_val}, max: {max_val}")
        if RecFaults:
            code = "MQ"
            threshold = 1e3
            count = mixed_qkv.numel()
            # faults_count = ((torch.abs(output) > threshold) | torch.isnan(output) | torch.isinf(output)).sum().item()
            n_inf = (torch.abs(mixed_qkv) > threshold).sum().item()
            nan = (torch.isnan(mixed_qkv)).sum().item()
            inf = (torch.isinf(mixed_qkv)).sum().item()
        ...
        # if get_tensor_model_parallel_rank() == 0: 
        #     print(f"hidden_states: {hidden_states.size()}, query: {query.size()}, key: {key.size()}, value: {value.size()}")
